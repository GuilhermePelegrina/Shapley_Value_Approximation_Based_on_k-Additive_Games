...
'''
dataset = pd.read_csv('games_adult_classification_random_forest.csv')
...
for jj in range(nSimul):
    ...
    for ii in range(len(samples_size)):
        ...
        error_all_k1, error_all_k2, error_all_k3, error_all_k4, count2, count3, count4 = mod_global_kadd_shapley.kadd_global_shapley(values_samples,samples_size,ii,jj,matrix_transf_k1,matrix_transf_k2,matrix_transf_k3,matrix_transf_k4,nAttr,samples[jj,:],inter_true,error_all_k2,error_all_k3,error_all_k1,error_all_k4,count2,count3,count4,index_k1,index_k2,index_k3,index_k4)
...
plt.plot(index_k1, np.mean(error_all_k1,0), label='1-Additive')
plt.plot(index_k2, np.mean(error_all_k2,0), label='2-Additive')
plt.plot(index_k3, np.mean(error_all_k3,0), label='3-Additive')
plt.plot(index_k4, np.mean(error_all_k4,0), label='4-Additive')
plt.ylim([0,0.005])
plt.legend()    
'''

'''
data_save = [samples,error_all_k1, error_all_k2, error_all_k3, error_all_k4, index_k1, index_k2, index_k3, index_k4]
